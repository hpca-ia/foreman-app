...
function AccesoDenegado({ msg }) {
  return (
    <div style={{textAlign:"center",padding:"60px 20px",color:"#9CA3AF"}}>
      ...
      <div style={{fontSize:12}}>{msg || "No tienes permiso para ver esta sección."}</div>
    </div>
  );
}
    ...
    if (n.length === 4) {
      setTimeout(() => {
        if (n === sel.pin) {
          ...
          saveLS("foreman_session", { userId:sel.id, expires:exp.toISOString() });
          onLogin(sel);
  ...
  return (
    <div style={{minHeight:"100vh",background:"#F8F9FB",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24,fontFamily:"'Inter',sans-serif"}}>
      ...
      <div style={{marginBottom:32,textAlign:"center"}}>
        ...
        <div style={{fontSize:12,color:"#9CA3AF"}}>Gestión de Obra · Sesión 7 días</div>
      </div>
      ...
      {step === "pick" && (
        <div style={{width:"100%",maxWidth:420}}>
          ...
          {users.map(u => {
            const b = rolBadge(u.role);
            return (
              <button key={u.id} onClick={() => selectUser(u)}
                style={{width:"100%",background:"#fff",border:"1.5px solid #E5E7EB",borderRadius:12,padding:"12px 16px",marginBottom:8,display:"flex",alignItems:"center",gap:14,cursor:"pointer",transition:"all 0.15s",fontFamily:"'Inter',sans-serif"}}
      ...
      {step === "pin" && sel && (
        <div style={{width:"100%",maxWidth:280,textAlign:"center",fontFamily:"'Inter',sans-serif"}}>
          <button onClick={() => { setStep("pick"); setErr(""); }}
            style={{background:"none",border:"none",color:"#9CA3AF",cursor:"pointer",fontSize:13,display:"flex",alignItems:"center",gap:4,margin:"0 auto 20px"}}>
            ← Volver
          </button>
          ...
          <div style={{display:"flex",justifyContent:"center",gap:16,marginBottom:28}}>
            {[0,1,2,3].map(i => (
              <div key={i} style={{width:12,height:12,borderRadius:"50%",background:pin.length>i?"#E8622A":"#E5E7EB",transition:"background 0.15s"}}/>
            ))}
          </div>
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          system:"Eres NOVA. Extrae datos y responde SOLO JSON sin markdown: {\"title\":\"...\",\"project_id\":N,\"assignee_id\":N,\"type\":\"...\",\"due_date\":\"YYYY-MM-DD\",\"priority\":\"urgente|alta|media|baja\",\"notes\":\"...\"}\nProyectos: "+proyList+". Usuarios: "+userList+".\nTipos: Llamada,Reunión,Contrato,Compra,Inspección,Aprobación,Visita a obra,Otro.\nHoy: "+new Date().toISOString().split("T")[0]+".",
  ...
  return (
    <div style={{background:"#fff",border:"1.5px solid #FED7AA",borderRadius:12,padding:14,marginBottom:14}}>
      ...
      <div style={{display:"flex",gap:8,marginBottom:8}}>
        <input value={texto} onChange={e => setTexto(e.target.value)} onKeyDown={e => e.key==="Enter" && procesar()}
          placeholder='"Inspección BdP Condado con Hector, urgente mañana"'
          style={{flex:1,background:"#FFF7F0",border:"1.5px solid #FED7AA",borderRadius:8,color:"#111",fontSize:13,padding:"8px 12px",outline:"none",fontFamily:"inherit"}}/>
        <button onClick={procesar} disabled={!texto.trim()||loading}
          style={{background:texto.trim()&&!loading?"#E8622A":"#F3F4F6",border:"none",borderRadius:8,padding:"8px 14px",color:texto.trim()&&!loading?"#fff":"#9CA3AF",fontSize:12,fontWeight:600,cursor:texto.trim()&&!loading?"pointer":"default",whiteSpace:"nowrap"}}>
          {loading?"...":"Crear →"}
        </button>
      </div>
      {result && !result.error && (
...
function WhatsApp({ task, users, projects }) {
  ...
  const m = users.find(u => u.id===task.assignee_id);
  const p = projects.find(p => p.id===task.project_id);
  const d = daysUntil(task.due_date);

  async function generar() {
    setLoading(true); setOpen(true);
    const vence = task.status==="listo"?"está completada":d<0?`${Math.abs(d)} días de retraso`:d===0?"vence HOY":`vence en ${d} días`;
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({model:"claude-sonnet-4-6",max_tokens:200,system:"Recordatorio WhatsApp construcción. Español. Máx 3 oraciones. Directo. Solo el mensaje.",messages:[{role:"user",content:`Para ${m?.name||"equipo"}: "${task.title}" en ${p?.name}. ${vence}. Prioridad: ${task.priority}.`}]})
      });
      const data = await res.json();
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.3)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:300,padding:20}} onClick={() => setOpen(false)}>
          <div style={{background:"#fff",borderRadius:16,padding:20,maxWidth:400,width:"100%",fontFamily:"'Inter',sans-serif"}} onClick={e => e.stopPropagation()}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
              ...
              <div><div style={{fontSize:14,fontWeight:700}}>WhatsApp</div>{m && <div style={{color:"#6B7280",fontSize:12}}>{m.name}</div>}</div>
            ...
            {loading ? <div style={{color:"#9CA3AF",fontSize:12,padding:"16px 0",textAlign:"center"}}>Redactando...</div> : (
              <>
                <textarea value={msg} onChange={e => setMsg(e.target.value)}
                  style={{width:"100%",minHeight:90,background:"#F9FAFB",border:"1.5px solid #E5E7EB",borderRadius:10,color:"#111",fontSize:13,padding:10,resize:"vertical",boxSizing:"border-box",outline:"none",fontFamily:"inherit"}}/>
...
function TarjetaTarea({ task, currentUser, users, projects, onCambiarEstado, onEditar, onEliminar }) {
  ...
  const puedeCambiar = perms.verTodasTareas || esMia;
      ...
      <div style={{display:"flex",gap:5,flexWrap:"wrap",alignItems:"center",paddingTop:6,borderTop:"1px solid #F3F4F6"}}>
        {puedeCambiar && Object.entries(ESTADO).map(([k,v]) => (
          <button key={k} onClick={() => onCambiarEstado(task.id,k)}
            style={{background:task.status===k?v.color:"#F3F4F6",border:"1px solid "+(task.status===k?v.color:"#E5E7EB"),borderRadius:6,padding:"4px 8px",color:task.status===k?"#fff":"#6B7280",fontSize:10,fontWeight:task.status===k?600:400,cursor:"pointer",whiteSpace:"nowrap"}}>
            {v.icon} {v.label}
          </button>
        ))}
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.25)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:150,padding:20}} onClick={onCerrar}>
      <div style={{background:"#fff",borderRadius:16,padding:22,maxWidth:480,width:"100%",maxHeight:"90vh",overflowY:"auto",fontFamily:"'Inter',sans-serif"}} onClick={e => e.stopPropagation()}>
        ...
        {!form.title||!form.due_date
          ? <div style={{color:"#9CA3AF",fontSize:11,marginTop:12,textAlign:"center"}}>Completa título y fecha</div>
          : <button onClick={() => { onGuardar(form,editTask?.id); onCerrar(); }} style={{width:"100%",marginTop:16,background:"#E8622A",border:"none",borderRadius:10,padding:12,color:"#fff",fontSize:14,fontWeight:600,cursor:"pointer"}}>
...
function ModuloPresupuesto({ proyectoId, proyectos, perms }) {
  ...
  return (
    <div>
      ...
      {rubros.map(r => {
        const e = ejec(r.id);
        const s = Number(r.presupuesto||0)-e;
        const rojo = Number(r.presupuesto)>0 && e>Number(r.presupuesto);
        return (
            ...
            {editando?.id===r.id && perms.editarPresupuesto ? (
              <div style={{display:"grid",gap:8}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                  <input value={editando.categoria} onChange={e2 => setEditando(p => ({...p,categoria:e2.target.value}))} style={iS} placeholder="Categoría"/>
              ...
              <>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                  <div>
                    ...
                    <div style={{fontSize:10,color:"#9CA3AF"}}>{r.categoria}</div>
                  </div>
        ...
        nuevo ? (
          <div style={{background:"#F9FAFB",borderRadius:10,padding:12,marginBottom:8,border:"1.5px solid #E8622A"}}>
            <div style={{display:"grid",gap:8}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                <input value={nForm.categoria} onChange={e => setNForm(p => ({...p,categoria:e.target.value}))} style={iS} placeholder="Categoría"/>
      ...
      {!perms.editarPresupuesto && (
        <div style={{background:"#F9FAFB",borderRadius:8,padding:"8px 12px",border:"1px dashed #E5E7EB",textAlign:"center"}}>
          <div style={{fontSize:11,color:"#9CA3AF"}}>🔒 Solo el Director puede agregar o editar rubros</div>
        </div>
      )}
    </div>
  );
...
function ModuloFacturas({ proyectoId, perms }) {
  ...
  async function clasificar() {
    ...
    setClas(true); setSug(null);
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-6",max_tokens:200,system:"Clasifica en rubro de construcción. Solo JSON sin markdown: {\"rubroId\":\"...\",\"razon\":\"...\"}. Rubros: "+rubros.map(r => r.id+"="+r.nombre).join(", "),messages:[{role:"user",content:form.descripcion}]})});
      const data = await res.json();
      setSug(JSON.parse((data.content?.[0]?.text||"{}").replace(/```json|```/g,"").trim()));
    } catch {}
    setClas(false);
  }
    ...
    <div>
      {perms.registrarFacturas && (
        <div style={{background:"#fff",border:"1.5px solid #E5E7EB",borderRadius:12,padding:14,marginBottom:14}}>
          ...
          <div style={{display:"grid",gap:8}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              <div><label style={lS}>Proveedor *</label><input value={form.proveedor} onChange={e => setForm(p => ({...p,proveedor:e.target.value}))} style={iS} placeholder="Proveedor"/></div>
              <div><label style={lS}>Monto USD *</label><input type="number" value={form.monto} onChange={e => setForm(p => ({...p,monto:e.target.value}))} style={iS} placeholder="0.00"/></div>
            </div>
            <div>
              ...
              <div style={{display:"flex",gap:8}}>
                ...
                <button onClick={clasificar} disabled={clas} style={{background:"#1F2937",border:"none",borderRadius:6,padding:"8px 12px",color:"#E8622A",fontSize:11,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",gap:4}}>
                  <NovaIcon size={14}/>{clas?"...":"Clasificar"}
                </button>
              </div>
              {sug && !sug.error && (
            ...
            <div><label style={lS}>Rubro *</label>
...
function ModuloCotizaciones({ proyectoId }) {
  ...
  async function analizar(c) {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-6",max_tokens:200,system:"Analiza cotización vs presupuesto. Solo JSON sin markdown: {\"tiene_ganancia\":true,\"margen_usd\":0,\"margen_pct\":0,\"alerta\":\"...\",\"recomendacion\":\"...\"}",messages:[{role:"user",content:"Rubro: "+rubro?.nombre+". Disponible: "+disp+". Cotización: "+c.monto+" de "+c.proveedor+"."}]})});
      const data = await res.json();
      const a = JSON.parse((data.content?.[0]?.text||"{}").replace(/```json|```/g,"").trim());
      save(cotiz.map(x => x.id===c.id?{...x,analisis:a}:x));
    } catch {}
    setAnal(null);
  }
  ...
  return (
    <div>
      <div style={{background:"#fff",border:"1.5px solid #E5E7EB",borderRadius:12,padding:14,marginBottom:14}}>
        <div style={{fontSize:13,fontWeight:600,marginBottom:10}}>📄 Nueva cotización</div>
        <div style={{display:"grid",gap:8}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          ...
          <input value={form.descripcion} onChange={e => setForm(p => ({...p,descripcion:e.target.value}))} style={iS} placeholder="Descripción"/>
          <select value={form.rubroId} onChange={e => setForm(p => ({...p,rubroId:e.target.value}))} style={iS}>
            <option value="">Rubro (opcional)...</option>
            {rubros.map(r => <option key={r.id} value={r.id}>{r.nombre}</option>)}
          </select>
          <button onClick={agregar} disabled={!form.proveedor||!form.monto} style={{background:form.proveedor&&form.monto?"#E8622A":"#F3F4F6",border:"none",borderRadius:8,padding:10,color:form.proveedor&&form.monto?"#fff":"#9CA3AF",fontSize:13,fontWeight:600,cursor:"pointer"}}>Registrar cotización</button>
        </div>
      </div>
      {cotiz.length===0
        ? <div style={{textAlign:"center",color:"#9CA3AF",padding:"40px 0"}}><div style={{fontSize:32,marginBottom:8}}>📄</div>Sin cotizaciones</div>
        : cotiz.map(c => {
          const rubro = rubros.find(r => r.id===c.rubroId);
          return (
              ...
              {c.analisis ? (
                <div style={{background:c.analisis.tiene_ganancia?"#F0FDF4":"#FEF2F2",borderRadius:8,padding:"8px 10px",border:"1px solid "+(c.analisis.tiene_ganancia?"#BBF7D0":"#FECACA")}}>
                  <div style={{fontSize:12,fontWeight:700,color:c.analisis.tiene_ganancia?"#059669":"#DC2626",marginBottom:3}}>
                    {c.analisis.tiene_ganancia?"✅ Dentro del presupuesto":"❌ Excede presupuesto"} · Margen: {fmt(c.analisis.margen_usd)} ({c.analisis.margen_pct}%)
                  </div>
                  <div style={{fontSize:11,color:"#374151"}}>{c.analisis.recomendacion}</div>
                </div>
              ) : (
                <button onClick={() => analizar(c)} disabled={anal===c.id} style={{background:"#1F2937",border:"none",borderRadius:6,padding:"6px 12px",color:"#E8622A",fontSize:11,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",gap:4}}>
                  <NovaIcon size={14}/>{anal===c.id?"Analizando...":"¿Hay ganancia?"}
                </button>
              )}
            </div>
          );
...
function ModuloCajaChica({ proyectoId, currentUser, perms }) {
  const rubros = loadLS("fin_rubros_"+proyectoId, RUBROS_DEFAULT);
  const [gastos, setGastos] = useState(() => loadLS("fin_caja_"+proyectoId,[]));
  const [saldo, setSaldo] = useState(() => loadLS("fin_caja_saldo_"+proyectoId,0));
  ...
  function saveG(g) { setGastos(g); saveLS("fin_caja_"+proyectoId,g); }
  const total = gastos.reduce((s,g) => s+Number(g.monto||0),0);
  const disp = Number(saldo)-total;
  const visibles = perms.verReporteCaja ? gastos : gastos.filter(g => g.responsable===currentUser.name);

  function agregar() {
    if (!form.descripcion||!form.monto) return;
    const resp = perms.verReporteCaja ? (form.responsable||currentUser.name) : currentUser.name;
    saveG([{...form,responsable:resp,id:Date.now(),monto:Number(form.monto),creadoEn:new Date().toISOString()},...gastos]);
  ...
  async function generar() {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-6",max_tokens:400,system:"Reporte caja chica construcción. Solo JSON sin markdown: {\"resumen\":\"...\",\"sin_factura\":0,\"pendiente_devolucion\":0,\"observaciones\":[\"...\"],\"estado\":\"ok|revisar|urgente\"}",messages:[{role:"user",content:"Saldo inicial: "+saldo+". Gastado: "+total+". Disponible: "+disp+". Gastos: "+JSON.stringify(gastos.map(g => ({descripcion:g.descripcion,monto:g.monto,responsable:g.responsable,tieneFactura:g.tieneFactura,rubro:rubros.find(r => r.id===g.rubroId)?.nombre||"Sin rubro"})))}]})});
      const data = await res.json();
    ...
    <div>
      {perms.verReporteCaja && (
        <div style={{background:"#fff",borderRadius:12,padding:"12px 14px",marginBottom:12,border:"1px solid #F0F1F3"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            ...
            <button onClick={generar} style={{background:"#1F2937",border:"none",borderRadius:6,padding:"6px 12px",color:"#E8622A",fontSize:11,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",gap:4}}>
              <NovaIcon size={14}/>Reporte
            </button>
          </div>
          <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:8}}>
            {[{l:"Saldo inicial",v:fmt(saldo),c:"#374151"},{l:"Gastado",v:fmt(total),c:"#D97706"},{l:"Disponible",v:fmt(disp),c:disp<0?"#DC2626":"#059669"}].map(s2 => (
              <div key={s2.l} style={{background:"#F9FAFB",borderRadius:8,padding:"8px 14px",textAlign:"center",flex:1,minWidth:80}}>
                <div style={{fontSize:15,fontWeight:700,color:s2.c}}>{s2.v}</div>
                <div style={{fontSize:10,color:"#9CA3AF",fontWeight:600}}>{s2.l}</div>
              </div>
            ))}
          </div>
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            <label style={{fontSize:11,color:"#6B7280",fontWeight:500,whiteSpace:"nowrap"}}>Ajustar saldo:</label>
            <input type="number" value={saldo} onChange={e => { setSaldo(Number(e.target.value)); saveLS("fin_caja_saldo_"+proyectoId,Number(e.target.value)); }} style={{...iS,maxWidth:140}} placeholder="USD"/>
          </div>
        </div>
      )}
      ...
      {!perms.verReporteCaja && (
        <div style={{background:"#EFF6FF",borderRadius:8,padding:"10px 14px",marginBottom:12,border:"1px solid #BFDBFE"}}>
          <div style={{fontSize:12,color:"#2563EB",fontWeight:600}}>📋 Registra tus gastos del día. El Director y Asistente revisan el saldo y hacen el cierre.</div>
        </div>
      )}
      ...
      <div style={{background:"#fff",border:"1.5px solid #E5E7EB",borderRadius:12,padding:14,marginBottom:14}}>
        ...
        <div style={{display:"grid",gap:8}}>
          ...
          <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",fontSize:12,color:"#374151"}}>
            ...
            Tiene factura o recibo de respaldo
          </label>
          <button onClick={agregar} disabled={!form.descripcion||!form.monto}
            style={{background:form.descripcion&&form.monto?"#E8622A":"#F3F4F6",border:"none",borderRadius:8,padding:10,color:form.descripcion&&form.monto?"#fff":"#9CA3AF",fontSize:13,fontWeight:600,cursor:"pointer"}}>
            Registrar gasto
          </button>
        </div>
      </div>
      ...
      {visibles.length===0
        ? <div style={{textAlign:"center",color:"#9CA3AF",padding:"30px 0"}}><div style={{fontSize:32,marginBottom:8}}>💳</div>Sin gastos registrados</div>
        : visibles.map(g => {
          const rubro = rubros.find(r => r.id===g.rubroId);
          return (
            <div key={g.id} style={{background:"#fff",borderRadius:10,padding:"10px 12px",marginBottom:6,border:"1px solid #F0F1F3",display:"flex",justifyContent:"space-between",gap:10}}>
              <div style={{flex:1}}>
      ...
      {modal && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.3)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:300,padding:20}} onClick={() => setModal(false)}>
          <div style={{background:"#fff",borderRadius:16,padding:22,maxWidth:460,width:"100%",maxHeight:"85vh",overflowY:"auto",fontFamily:"'Inter',sans-serif"}} onClick={e => e.stopPropagation()}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:16}}>
              <NovaIcon size={24}/>
              <div style={{fontSize:14,fontWeight:700}}>Reporte de Caja Chica</div>
              <button onClick={() => setModal(false)} style={{marginLeft:"auto",background:"none",border:"none",color:"#9CA3AF",cursor:"pointer",fontSize:20}}>×</button>
            </div>
              ...
              : rep && !rep.error && (
                <div>
                  <div style={{background:rep.estado==="ok"?"#F0FDF4":rep.estado==="revisar"?"#FEF3C7":"#FEF2F2",borderRadius:8,padding:"10px 14px",marginBottom:14,border:"1px solid "+(rep.estado==="ok"?"#BBF7D0":rep.estado==="revisar"?"#FDE68A":"#FECACA")}}>
                    <div style={{fontSize:12,fontWeight:700,color:rep.estado==="ok"?"#059669":rep.estado==="revisar"?"#D97706":"#DC2626",marginBottom:4}}>
                      {rep.estado==="ok"?"✅ OK":rep.estado==="revisar"?"⚠️ Revisar":"🔴 Urgente"}
                    </div>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
                    ...
                    <div style={{background:"#F0FDF4",borderRadius:8,padding:"10px 14px",textAlign:"center"}}>
                      <div style={{fontSize:18,fontWeight:700,color:"#059669"}}>{fmt(rep.pendiente_devolucion||0)}</div>
                      <div style={{fontSize:10,color:"#059669",fontWeight:600}}>A DEVOLVER</div>
                    </div>
                  </div>
                  {rep.observaciones?.length>0 && rep.observaciones.map((o,i) => (
                    <div key={i} style={{fontSize:12,color:"#6B7280",marginBottom:4,paddingLeft:12,borderLeft:"2px solid #E5E7EB"}}>{o}</div>
                  ))}
                  <button onClick={() => window.print()} style={{width:"100%",marginTop:16,background:"#1F2937",border:"none",borderRadius:8,padding:10,color:"#fff",fontSize:13,fontWeight:600,cursor:"pointer"}}>🖨 Imprimir</button>
                </div>
              )}
          </div>
        </div>
...
// ── MÓDULO PROYECTO (router de tabs) ─────────────────────────
...
export default function App() {
  ...
  const [projects] = useState(() => loadLS("foreman_projects", PROJECTS_DEFAULT));
  ...
  return (
    <div style={{minHeight:"100vh",background:"#F8F9FB",fontFamily:"'Inter',sans-serif",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{background:"#fff",borderBottom:"1.5px solid #F0F1F3",padding:"0 16px",position:"sticky",top:0,zIndex:100,boxShadow:"0 1px 4px rgba(0,0,0,0.04)"}}>
        <div style={{display:"flex",alignItems:"center",height:54,gap:12,maxWidth:1100,margin:"0 auto",width:"100%"}}>
          ...
          <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:8}}>
            ...
            <span style={{background:b.bg,color:b.color,fontSize:10,fontWeight:700,padding:"2px 8px",borderRadius:20}}>{b.label}</span>
            {perms.verAjustes && <button onClick={() => setShowAjustes(true)} style={{background:"#F3F4F6",border:"none",borderRadius:8,padding:"6px 10px",color:"#6B7280",fontSize:12,cursor:"pointer"}}>⚙️</button>}
            ...
            {vista==="tareas" && perms.crearTareas && (
              <button onClick={() => { setEditTask(null); setShowModal(true); }} style={{background:"#E8622A",border:"none",borderRadius:8,padding:"7px 14px",color:"#fff",fontSize:13,fontWeight:600,cursor:"pointer",boxShadow:"0 4px 12px rgba(232,98,42,0.25)",whiteSpace:"nowrap"}}>
                + Nueva tarea
              </button>
            )}
          </div>
          ...
          {vista==="tareas" && (
            <>
              ...
              {cargando
                ...
                : visibles.length===0
                  ? <div style={{textAlign:"center",color:"#9CA3AF",padding:"60px 0"}}><div style={{fontSize:36,marginBottom:10}}>🏗</div>{perms.crearTareas?"Sin tareas. Toca \"+ Nueva tarea\" o dile a NOVA.":"No tienes tareas asignadas aún."}</div>
                  : visibles
                      .sort((a,b2) => {
                        const o={urgente:0,alta:1,media:2,baja:3};
                        if (a.status==="listo"&&b2.status!=="listo") return 1;
                        if (b2.status==="listo"&&a.status!=="listo") return -1;
                        return (o[a.priority]-o[b2.priority])||(daysUntil(a.due_date)-daysUntil(b2.due_date));
                      })
                      .map(t => (
                        <TarjetaTarea key={t.id} task={t} currentUser={usuario} users={users} projects={projects}
                          onCambiarEstado={cambiarEstado}
                          onEditar={t2 => { setEditTask(t2); setShowModal(true); }}
                      ...
                      <div key={m.id} style={{background:"#fff",borderRadius:12,padding:14,border:"1px solid #E5E7EB"}}>
                        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:mt.length?12:0}}>
                          ...
                          <div style={{flex:1}}>
                            ...
                            <span style={{background:mb.bg,color:mb.color,fontSize:10,fontWeight:700,padding:"2px 6px",borderRadius:10}}>{mb.label}</span>
                          </div>
                          <div style={{textAlign:"right"}}>
                        ...
                        {mt.map(t => (
                          <div key={t.id} style={{background:"#F9FAFB",borderRadius:8,padding:"7px 10px",marginBottom:5,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                            <div><div style={{fontSize:12,fontWeight:500}}>{t.title}</div><div style={{fontSize:11,color:"#9CA3AF"}}>{projects.find(p => p.id===t.project_id)?.name}</div></div>
            ...
            perms.verProyectos
              ? <div style={{display:"grid",gap:10}}>
                  {projects.map(p => {
                    ...
                    const av = pt.length>0?Math.round((pOk.length/pt.length)*100):0;
                    return (
                      <div key={p.id} style={{background:"#fff",borderRadius:12,padding:14,border:"1px solid #E5E7EB",borderLeft:"4px solid "+p.color}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                          ...
                          <div style={{display:"flex",gap:8,alignItems:"center"}}>
                            ...
                            <button onClick={() => { setVista("finanzas-proyecto"); setProyFin(p.id); }} style={{background:"#FFF4F0",border:"1px solid #FED7AA",borderRadius:6,padding:"4px 10px",color:"#E8622A",fontSize:11,fontWeight:600,cursor:"pointer"}}>💰 Finanzas</button>
                          </div>
                        </div>
          ...
          {vista==="finanzas-dashboard" && (
            perms.verDashboard
              ? <div>
                  ...
                  {projects.map(p => {
                    const rubros = loadLS("fin_rubros_"+p.id, RUBROS_DEFAULT);
                    const facts  = loadLS("fin_facturas_"+p.id, []);
                    const presup = rubros.reduce((s,r) => s+Number(r.presupuesto||0),0);
                    const ejec   = facts.reduce((s,f) => s+Number(f.monto||0),0);
                    if (presup===0) return null;
                    const sd = presup-ejec;
                    return (
                      <div key={p.id} onClick={() => { setVista("finanzas-proyecto"); setProyFin(p.id); }}
                        style={{background:"#fff",borderRadius:12,padding:"12px 16px",marginBottom:10,border:"1px solid #F0F1F3",borderLeft:"4px solid "+p.color,cursor:"pointer"}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                          <div style={{fontWeight:700,fontSize:14}}>{p.name}</div>
                          <span style={{background:sd<0?"#FEE2E2":"#F0FDF4",color:sd<0?"#DC2626":"#059669",fontSize:12,fontWeight:700,padding:"2px 10px",borderRadius:20}}>
                            {sd<0?"⚠ -"+fmt(Math.abs(sd)):"✓ +"+fmt(sd)}
                          </span>
                        </div>
                  ...
                  <div style={{background:"#F9FAFB",borderRadius:8,padding:"10px 14px",border:"1px dashed #E5E7EB",textAlign:"center"}}>
                    <div style={{fontSize:11,color:"#9CA3AF"}}>Selecciona un proyecto en el menú lateral para ver el detalle.</div>
                  </div>
                </div>
              : <AccesoDenegado msg="Solo Director y Asistente pueden ver el dashboard financiero."/>
          )}
      ...
      {showModal && (
        <ModalTarea editTask={editTask} currentUser={usuario} users={users} projects={projects}
          onCerrar={() => { setShowModal(false); setEditTask(null); }}
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.3)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:150,padding:20}} onClick={() => setShowAjustes(false)}>
          <div style={{background:"#fff",borderRadius:16,padding:22,maxWidth:440,width:"100%",maxHeight:"90vh",overflowY:"auto",fontFamily:"'Inter',sans-serif"}} onClick={e => e.stopPropagation()}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
              <div style={{fontSize:15,fontWeight:700}}>⚙️ Ajustes</div>
              <button onClick={() => setShowAjustes(false)} style={{background:"#F3F4F6",border:"none",borderRadius:6,width:28,height:28,color:"#6B7280",cursor:"pointer",fontSize:15}}>×</button>
            </div>
            {[{k:"nombre",l:"Empresa",ph:"HCA Studio"},{k:"email",l:"Email",ph:"info@hcastudio.com"},{k:"ciudad",l:"Ciudad",ph:"Quito"},{k:"moneda",l:"Moneda",ph:"USD"}].map(f => (
              <div key={f.k} style={{marginBottom:10}}>
                ...
                <input value={empresa?.[f.k]||""} onChange={e => { const ne={...empresa,[f.k]:e.target.value}; setEmpresa(ne); saveLS("foreman_empresa",ne); }} placeholder={f.ph}
              ...
              {users.map(u => {
                ...
                return (
                  <div key={u.id} style={{background:"#F9FAFB",borderRadius:10,padding:"10px 12px",marginBottom:8,display:"flex",alignItems:"center",gap:10}}>
                    ...
                    <div style={{flex:1}}><div style={{fontSize:13,fontWeight:600}}>{u.name}</div><div style={{fontSize:11,color:"#9CA3AF"}}>PIN: {u.pin}</div></div>
                    <span style={{background:ub.bg,color:ub.color,fontSize:10,fontWeight:700,padding:"2px 6px",borderRadius:10}}>{ub.label}</span>
                  </div>
                );
              })}
            </div>
