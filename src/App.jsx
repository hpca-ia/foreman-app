...
export default function App() {
  ...
  useEffect(() => {
    if (!usuario) return;
    fetchTareas();
    const taskChannel = supabase.channel("tasks-realtime")
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "tasks" },
        payload => { setTareas(prev => [payload.new, ...prev]); })
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "tasks" },
        payload => setTareas(prev => prev.map(t => t.id === payload.new.id ? payload.new : t)))
      .on("postgres_changes", { event: "DELETE", schema: "public", table: "tasks" },
        payload => setTareas(prev => prev.filter(t => t.id !== payload.old.id)))
      .subscribe();
    const commentChannel = supabase.channel("comments-count-realtime")
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "comments" },
        payload => setCommentCounts(prev => ({ ...prev, [payload.new.task_id]: (prev[payload.new.task_id] || 0) + 1 })))
      .subscribe();
    return () => { supabase.removeChannel(taskChannel); supabase.removeChannel(commentChannel); };
  }, [usuario]);
